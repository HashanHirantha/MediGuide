...
BEGIN
    ...
    SELECT id INTO doctor_uuid FROM auth.users WHERE email = 'dr.smith@mediguide.com';

    IF doctor_uuid IS NULL THEN
        doctor_uuid := gen_random_uuid();
        ...
        VALUES (
            doctor_uuid, 'authenticated', 'authenticated', 'dr.smith@mediguide.com', extensions.crypt('password123', extensions.gen_salt('bf')),
            now(), now(), now(),
